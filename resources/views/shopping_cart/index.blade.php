        ...
        <div class="card border shadow-0">
          <div class="m-4">
            ...
            @foreach($shoppingCarts as $shoppingCart)
              @php $total += $shoppingCart->product->product_price * $shoppingCart->quantity_sub_product @endphp
            <div class="row gy-3 mb-4">
              <div class="col-lg-5">
                <div class="me-lg-5">
                  <div class="d-flex">
                    <img src="{{asset("storage/product/{$shoppingCart->product->image_product}")}}" class="border rounded me-3" style="width: 96px; height: 96px;" />
                    <div class="">
                      <a href="#" class="nav-link">{{$shoppingCart->product->product_name}}</a>
                      <p class="text-muted">{{$shoppingCart->product->short_description}}</p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="col-lg-2 col-sm-6 col-6 d-flex flex-row flex-lg-column flex-xl-row text-nowrap gap-5">
                <div class="">
                 {{$shoppingCart->quantity_sub_product}}
                </div>
                <div class="">
                  <text class="h6">Rp {{$shoppingCart->quantity_sub_product * $shoppingCart->product->product_price}}</text> <br />
                  <small class="text-muted text-nowrap"> Rp {{$shoppingCart->product->product_price}} / per item </small>
                </div>
              </div>
      ...
      <div class="col-lg-3">
        ...
        <div class="card shadow-0 border">
          <div class="card-body">
            ...
            <div class="d-flex justify-content-between">
              <p class="mb-2">Total price:</p>
              <p class="mb-2 fw-bold">Rp {{$total}}</p>
            </div>

            <div class="mt-3">
              <form method="POST" class="d-inline-block col-md-12" action="{{ url('sales-invoice') }}">
                @csrf
                <input class="d-none" type="text" name="shipping_id" value="{{1}}" />
                @error('shipping_id')
